/**
 * ApplyPilot — Seed test data
 * Run from the project root: node database/seed_test_data.mjs
 */
...
async function run() {
  ...
  // 1. Reset passwords + confirm emails
  ...
  for (const acc of ACCOUNTS) {
    ...
    console.log(error ? `❌ users: ${acc.email} — ${error.message}` : `✅ users: ${acc.email}`)
  }

  // 3. Get or create agencies
  let premierAgencyId, edupathAgencyId
  const { data: agencies } = await db.from('agencies').select('id, name')
  if (agencies) {
    for (const ag of agencies) {
      if (ag.name.toLowerCase().includes('premier')) premierAgencyId = ag.id
      if (ag.name.toLowerCase().includes('edupath')) edupathAgencyId = ag.id
    }
  }

  if (!premierAgencyId) {
    const { data, error } = await db.from('agencies').insert({ name: 'Premier Education', primary_color: '#1D9E75' }).select('id').single()
    premierAgencyId = data?.id
    console.log(error ? `❌ agency: Premier — ${error.message}` : `✅ agency: Premier Education created (${premierAgencyId})`)
  } else {
    console.log(`✅ agency: Premier Education exists (${premierAgencyId})`)
  }

  if (!edupathAgencyId) {
    const { data, error } = await db.from('agencies').insert({ name: 'EduPath Global', primary_color: '#2563EB' }).select('id').single()
    edupathAgencyId = data?.id
    console.log(error ? `❌ agency: EduPath — ${error.message}` : `✅ agency: EduPath Global created (${edupathAgencyId})`)
  } else {
    console.log(`✅ agency: EduPath Global exists (${edupathAgencyId})`)
  }
  ...
  for (const m of members) {
    if (!m.agency_id) { console.log(`⚠️  Skipping member ${m.user_id} — agency not found`); continue }
  ...
  console.log('\n✅ Done! Try logging in at http://localhost:3000/login')
}
